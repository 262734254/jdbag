...
public class JDCategoryDaoBean extends BaseDaoImpl<JDCategory>{
	...
	public void insertJdBrandXiu(List<XiuJdBrand> list)throws Exception{
		for (XiuJdBrand xiuJdBrand :list) {
			XiuJdBrand bean = (XiuJdBrand) getSqlMapClientTemplate().queryForObject("JDCategoryDaoBean.queryBrand2Xiu", xiuJdBrand);
			if(bean==null){
				getSqlMapClientTemplate().insert("JDCategoryDaoBean.insertJdBrandXiu",xiuJdBrand);
			}
		}
	}
	public void insertEbayJdBrand(List<XiuJdBrand> list)throws Exception{
		for (XiuJdBrand xiuJdBrand :list) {
			XiuJdBrand bean = (XiuJdBrand) getSqlMapClientTemplate().queryForObject("JDCategoryDaoBean.queryEbayBrand2", xiuJdBrand);
			if(bean==null){
				getSqlMapClientTemplate().insert("JDCategoryDaoBean.insertEbayJdBrand",xiuJdBrand);
			}
		}
	}
