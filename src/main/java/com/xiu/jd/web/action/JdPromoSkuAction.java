...
@Scope("prototype")
public class JdPromoSkuAction extends BaseAction {
private static final Logger logger=Logger.getLogger(JdPromoSkuAction.class);
	...
	private PageView<JdPromoSkuRec> pageView;
	...
	public String importPromoSkuUI(){
		Map<String,Object> parames =new HashMap<String,Object>();
		parames.put("promoId", promoId);
		this.setPageSize(20);
		pageView = new PageView<JdPromoSkuRec>(this.getPageSize(), this.getCurrentPage());
		parames.put("start", pageView.getFirstResult());
		parames.put("end", pageView.getMaxresult());
		QueryResult<JdPromoSkuRec> qr = jdPromoServiceBean.getPageJdSkuRecResule(parames);
		pageView.setQueryResult(qr);
		return SUCCESS;
	  ...
	  public String importPromoSku(){
			...
			if(uploadFile!= null){
				...
				try {
					...
					List<JdPromoSku> JdPromoSkus = new ArrayList<JdPromoSku>();
					for(int i=1;i<=rowCount;i++){
						
						JdPromoSku jdPromoSku =new JdPromoSku();
						String xiuCode=getSkuMapInfo(sheet,row,i,0);
						String xiuSku=getSkuMapInfo(sheet,row,i,1);
						String wareId=getSkuMapInfo(sheet,row,i,2);
						String jdWareSkuId=getSkuMapInfo(sheet,row,i,3);
						String jdPrice=getSkuMapInfo(sheet,row,i,4);
						...
						SellerPromotionSkuAddRequest skuAddRequest=new SellerPromotionSkuAddRequest();
						skuAddRequest.setPromoId(Long.valueOf(promoId));
						skuAddRequest.setSkuIds( jdPromoSku.getJdSkuId() );
						skuAddRequest.setJdPrices( jdPromoSku.getJdPrice() );
						SellerPromotionSkuAddResponse skuResponse=null;
						if (promoId!=null) {
							try {
								logger.info("==创建单品促销添加sku开始=="+promoId+" "+jdPromoSku);
								skuResponse = client.execute(skuAddRequest);
								if(skuResponse.getCode().equals("0")){
									jdPromoSku.setStatus(1);
								}
								else{
									jdPromoSku.setStatus(2);
									jdPromoSku.setFailDesc(skuResponse.getZhDesc());
								}
								
							} catch (Exception e) {
								jdPromoSku.setStatus(2);
								jdPromoSku.setFailDesc("单品添加sku失败："+skuResponse.getZhDesc());
								logger.error("单品添加sku促销失败：" + promoId + " "
										+ jdPromoSku.getJdSkuId() + "返回错误代码"
										+ skuResponse.getCode() + " 错误描述："
										+ skuResponse.getZhDesc());
								e.printStackTrace();
						...
						JdPromoSkus.add(jdPromoSku);
						
					}
					
					SellerPromotionCommitRequest requestCommit = new SellerPromotionCommitRequest();
					requestCommit.setPromoId(Long.valueOf(promoId));
					SellerPromotionCommitResponse responseCommit = null;
					int successCount=0;
					try {
						responseCommit = client
								.execute(requestCommit);
						if(responseCommit.getCode().equals("0")){
							for (JdPromoSku jdPromoSku2 : JdPromoSkus) {
								if(jdPromoSku2.getStatus()==1){
									successCount++;
								}
								try {
									jdPromoServiceBean
											.addPromoSku(jdPromoSku2);
								} catch (Exception e) {
									logger.error("促销编号："+promoId+" sku"+jdPromoSku2.getJdSkuId()+"入库失败");
									e.printStackTrace();
								}
							}
							JdPromoSkuRec jdPromoSkuRec =new JdPromoSkuRec();
							jdPromoSkuRec.setPromoId(promoId);
							jdPromoSkuRec.setFileName(uploadFileFileName);
							jdPromoSkuRec.setFileType("1");
							jdPromoSkuRec.setSuccessCount(String.valueOf(successCount));
							jdPromoSkuRec.setImportCount(String.valueOf(JdPromoSkus.size()));
							jdPromoSkuRec.setFailCount(String.valueOf(JdPromoSkus.size()-successCount));
							jdPromoServiceBean.insertJdPromoSkuRec(jdPromoSkuRec);
							
						}
					} catch (Exception e) {
						e.printStackTrace();
						logger.info("促销提交失败："+promoId);
					}
					request.setAttribute("message", "文件已经上传");
				} catch (Exception e) {
					request.setAttribute("message", "文件上传异常");
	  ...
	  public String getJdPromoSkuRecList(){
		  Map<String,Object> parames =new HashMap<String,Object>();
	      String redirectUrl = request.getContextPath() + "/jdGoods/importPromoSkuUI.action";
		  parames.put("promoId", promoId);
		  this.setPageSize(20);
	...
	public PageView<JdPromoSkuRec> getPageView() {
		return pageView;
	}

	public void setPageView(PageView<JdPromoSkuRec> pageView) {
